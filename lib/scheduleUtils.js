...
export function recurringAppliesToDate(recurring, dateStr) {
  ...
  if (recurring.week_pattern === 'every') return true
 ...
 * combining one-time shifts and recurring schedules.
 *
 ...
 * @returns {Set<string>}
 */
export function getEffectiveProviderIds(dateStr, shiftTime, oneTimeShifts, recurringRows) {
  const ids = new Set()
  for (const s of oneTimeShifts) {
    if (s.shift_date === dateStr && s.shift_time === shiftTime) ids.add(s.provider_id)
  }
  for (const r of recurringRows) {
    if (r.shift_time === shiftTime && recurringAppliesToDate(r, dateStr)) ids.add(r.provider_id)
  }
  return ids
}
 ...
 */
export function getEffectiveProviders(dateStr, shiftTime, oneTimeShifts, recurringRows) {
  const map = new Map()
  for (const s of oneTimeShifts) {
    if (s.shift_date === dateStr && s.shift_time === shiftTime) {
      map.set(s.provider_id, { id: s.provider_id, full_name: s.profiles?.full_name || '?', source: 'onetime' })
    }
  }
  for (const r of recurringRows) {
    if (r.shift_time === shiftTime && recurringAppliesToDate(r, dateStr) && !map.has(r.provider_id)) {
