...
export default function Pipeline({ supabase, profile, onVolunteerCreated }) {
  ...
  // ── Pre-computed available slots per waitlist entry (keyed by entry.id) ──
  // Avoids calling async function during render
  ...
  useEffect(() => {
    if (waitlist.length > 0 && schedule.length >= 0) {
      computeAllAvailableSlots(waitlist, schedule)
    }
  }, [waitlist, schedule])
  ...
  // ─── Compute available slots (pure, synchronous, uses passed-in schedule) ─
  // Returns array of { key, day, shift, role, filled }
  // Roles with no ROLE_SUGGESTIONS entry (unlimited) are always available.
  function computeAvailableSlots(entry, currentSchedule) {
    ...
    for (const key of slotKeys) {
      ...
      for (const role of roles) {
        ...
        // If limit is defined and > 0, check capacity. If undefined/null (unlimited), always open.
        if (limit !== undefined && limit !== null && limit > 0) {
          ...
          if (effectiveCount >= limit) continue // slot is full
          result.push({ key, day, shift, role, filled: effectiveCount })
        } else if (limit === 0) {
          // Explicitly 0 means never assign
          continue
        } else {
          // No limit defined — unlimited role, always available
  ...
  // Pre-compute for all waitlist entries and store in state
  function computeAllAvailableSlots(wl, sched) {
    const map = {}
    for (const entry of wl) {
      map[entry.id] = computeAvailableSlots(entry, sched)
    }
  ...
  // Opens assign modal, pre-selects first available slot (uses pre-computed map)
  ...
  async function handleAssignSlot() {
    ...
    // Re-query fresh at moment of assignment to prevent races
    const { data: freshSchedule } = await supabase.from('schedule').select('*')
    const limit = ROLE_SUGGESTIONS[assignRole]

    // Only enforce limit if it's explicitly defined and > 0
    if (limit !== undefined && limit !== null && limit > 0) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {waitlistError && (
          <div style={{ padding: '0.85rem 1rem', borderRadius: '10px', background: C.red + '08', border: `1px solid ${C.red}33` }}>
            ...
            <p style={{ fontSize: '0.78rem', color: 'var(--muted)' }}>Make sure you've run the migrations.sql file and that the RLS policies are applied.</p>
          </div>
        )}
        ...
        {waitlistLoading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            {filteredWaitlist.map((entry, idx) => {
              ...
              // Use pre-computed slots — always an array, never a Promise
              const slots    = availableSlotsMap[entry.id] || []
  ...
  function AssignModal() {
    ...
    // Use pre-computed slots for this entry
    const slots      = availableSlotsMap[assignModal.id] || []
    const uniqueKeys = [...new Set(slots.map(s => s.key))]

    // Roles for currently selected slot key (from pre-computed slots)
    const rolesForSlot = slots.filter(s => s.key === assignSlotKey)

    // All roles available for the modal — for unlimited roles (not in ROLE_SUGGESTIONS
    // or explicitly null/undefined), show all ROLES as fallback in the role picker
    const availableRoles = rolesForSlot.length > 0
      ? rolesForSlot.map(s => s.role)
      : ROLES
      ...
      <div onClick={() => setAssignModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: '1.5rem' }}>
        <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '16px', padding: '1.75rem', maxWidth: '520px', width: '100%', boxShadow: '0 24px 60px rgba(0,0,0,0.4)', maxHeight: '85vh', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', marginBottom: '1.25rem' }}>
            ...
            {/* Full slot grid — always shown when no prefs or no matching slots */}
            {(uniqueKeys.length === 0 || !assignModal.preferred_slots || assignModal.preferred_slots.length === 0) && (
              <div style={{ padding: '1rem', borderRadius: '10px', background: 'var(--bg)', border: `1px solid ${C.blue}22`, overflowX: 'auto' }}>
                ...
                <SlotPicker
                  ...
                  onChange={keys => {
                    if (keys.length > 0) {
                      setAssignSlotKey(keys[keys.length - 1])
