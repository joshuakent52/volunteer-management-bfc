...
// ProviderCredentialsBanner
// Drop this into your volunteer page for providers.
// Usage:  <ProviderCredentialsBanner profile={profile} />
// It renders nothing if the user is not a provider.
...
export default function ProviderCredentialsBanner({ profile }) {
  ...
  return (
    <div style={{ borderRadius: '12px', border: `1px solid ${borderColor}`, background: bgColor, padding: '1rem 1.25rem', marginBottom: '1rem' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.55rem' }}>
        {fields.map(f => {
          ...
          return (
            <div key={f.key} style={{ display: 'flex', flexDirection: 'column', gap: '0.15rem', padding: '0.5rem 0.75rem', borderRadius: '8px', border: `1px solid ${chipBorder}`, background: chipBg, minWidth: '90px' }}>
              ...
              <span style={{ fontSize: '0.8rem', fontWeight: isMissing ? 400 : 600, fontFamily: isMissing ? 'DM Sans, sans-serif' : 'DM Mono, monospace', color: dateColor, fontStyle: isMissing ? 'italic' : 'normal' }}>
                {isMissing ? 'Not on file' : formatExpDate(f.value)}
              </span>
              {isExpired  && <span style={{ fontSize: '0.65rem', color: '#ef4444', fontWeight: 700, lineHeight: 1 }}>EXPIRED</span>}
              {isExpiring && <span style={{ fontSize: '0.65rem', color: '#f97316', fontWeight: 700, lineHeight: 1 }}>EXP. SOON</span>}
            </div>
          )
        })}
      </div>
